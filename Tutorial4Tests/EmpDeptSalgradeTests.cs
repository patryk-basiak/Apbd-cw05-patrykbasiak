...
public class EmpDeptSalgradeTests
{
    ...
    public void ShouldReturnEmployeesFromChicago()
    {
        ...
        List<Emp> result = (from emp in emps
            where emp.DeptNo == (from dept in depts where dept.Loc == "CHICAGO" select dept.DeptNo).ToArray()[0]
            select emp).ToList();
    ...
    public void ShouldSelectNamesAndSalaries()
    {
        ...
        var result = emps.Select(r => new { r.EName, r.Sal }); 
    ...
    public void ShouldJoinEmployeesWithDepartments()
    {
        ...
        var result = from emp in emps
            ...
            select new {
                EName = emp.EName, DName = dept.DName
        };
    ...
    public void ShouldMatchEmployeeToSalaryGrade()
    {
        ...
        var result = from emp in emps
            ...
            select new
            {
                EName = emp.EName, Grade = grade.Grade
            };
    ...
    public void ShouldCalculateAverageSalaryPerDept()
    {
        ...
        var result = from emp in emps
            ...
            select new
            {
                DeptNo = g.Key, AvgSal = g.Average(emp => emp.Sal)
            };
    ...
    public void ShouldReturnEmployeesEarningMoreThanDeptAverage()
    {
        ...
        var result = from emp in emps
            let deptAvg = (from e in emps
                where e.DeptNo == emp.DeptNo
                select e.Sal).Average()
            where emp.Sal > deptAvg
            select emp.EName;
